...
function ToDoItem({ item }) {
    ...
    const handleDone = (e) => {
        e.target.parentNode.classList.toggle('done-todo')
    }
...

    {
        return (
            ...
            <div className="todo-container">
                ...
                {(!itemOwn.description?.startsWith(' ') && itemOwn.description?.length > 0 && !itemOwn.description?.startsWith(';º`?')) && <div className="todo-description">{itemOwn.description}</div>}
                {!itemOwn.date?.startsWith('u') && <div className="todo-date">{itemOwn.date}</div>}
                <button onClick={e => handleDone(e)} className="done-btn">Done</button>
